...
int main(int argc, char** argv)
{
    ...
    Nr = 256; // AES-256 by default.
    Nk = Nr / 32;
    Nr = Nk + 6;

    unsigned char* temp = (unsigned char*)malloc(32 * sizeof(unsigned char)); //{0x00  ,0x01  ,0x02  ,0x03  ,0x04  ,0x05  ,0x06  ,0x07  ,0x08  ,0x09  ,0x0a  ,0x0b  ,0x0c  ,0x0d  ,0x0e  ,0x0f};
    unsigned char* temp2 = (unsigned char*)malloc(32 * sizeof(unsigned char)); //{0x00  ,0x11  ,0x22  ,0x33  ,0x44  ,0x55  ,0x66  ,0x77  ,0x88  ,0x99  ,0xaa  ,0xbb  ,0xcc  ,0xdd  ,0xee  ,0xff};
    ...
    for (itr = 0; itr < numIterations; itr++)
    {
        ...
        // Seed the plaintext and ciphertext
        for (i = 0; i < 32; i++)
        {
             temp[i] = rand() % 256;
             temp2[i] = rand() % 256;
